...
DROP FUNCTION IF EXISTS public.get_portfolio_sector_industry_allocation();
DROP FUNCTION IF EXISTS public.get_portfolio_country_allocation();
DROP FUNCTION IF EXISTS public.get_portfolio_currency_allocation();
DROP FUNCTION IF EXISTS public.get_portfolio_asset_type_allocation();
DROP FUNCTION IF EXISTS public.get_latest_portfolio_holdings();
DROP FUNCTION IF EXISTS public.get_latest_portfolio_holdings_grouped();
DROP FUNCTION IF EXISTS public.get_latest_asset_dividend_performance();
DROP FUNCTION IF EXISTS public.get_latest_portfolio_holdings_grouped();
DROP FUNCTION IF EXISTS public.get_latest_holding_dividend_performance();
DROP FUNCTION IF EXISTS public.get_latest_asset_performance_percentage();
...
-- 2) get_portfolio_snapshot_at_date_or_before(p_snapshot_date date, p_platform text DEFAULT NULL)
...
-- 3) get_latest_portfolio_dividend_performance(p_platform text DEFAULT NULL)
...
STABLE
AS $$
    WITH latest AS (SELECT MAX(snapshot_date) AS d FROM asset_daily_snapshots WHERE (p_platform IS NULL OR platform = p_platform)),
                first_investment AS (
